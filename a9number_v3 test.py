...
def count_occurrences_in_text(word_or_phrase, text):
    """
    Return the number of occurrences of the passed word or phrase (case insensitive) in text.
    ...
    # Modifiez l'expression régulière pour capturer correctement les mots et la ponctuation
    pattern = r"[\w'-]+|[.,;:!?'\"]"

    # Extraire les éléments de la phrase de recherche
    word_list = re.findall(pattern, word_or_phrase)
    
    # Extraire tous les éléments du texte
    words = re.findall(pattern, text)
    print("All elements from text:", words)
    ...
    for i in range(len(words) - len(word_list) + 1):
        ...
        print(f"Comparing sequence '{sequence_str}' with '{word_or_phrase}'")
        if sequence == word_list:
            print("Match found! Incrementing count.")
            count += 1

    print(f"Final count for word or phrase: {count}")
    return count
