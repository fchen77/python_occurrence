...
def count_occurrences_in_text(word_or_phrase, text):
    ...
    # Modify the regular expression to correctly capture words and punctuation
    pattern = r"[\w'-]+|[.,;:!?'\"]"
    

    return text.count(word_or_phrase)

"""     # Extract elements from the search phrase
    word_list = re.findall(pattern, word_or_phrase)
    
    # Extract all elements from the text
    words = re.findall(pattern, text)
    
    count = 0
    for i in range(len(words) - len(word_list) + 1):
        sequence = words[i:i+len(word_list)]
        
        if sequence == word_list:
            count += 1
